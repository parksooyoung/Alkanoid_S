...
//		1. super init first
		...
		this.box_g.scaleX = 0.08;
		this.box_g.scaleY = 0.08;
		this.box_g.x = this.width/2;
		this.box_g.y = 300;
		this.addChild(this.box_g);
		
		var string = "Life : " + life.toString();
		var label = cc.LabelTTF("", "Arial", 30);
		...
		this.update = function(){
			...
			//초록박스 왼쪽 튕김 
			if(ballRight < box_g_Left && ballRight + this.ball.speedX > box_g_Left){
				//cc.log("Left");
				var xOnBox = this.ball.y + (box_g_Left - this.ball.getBoundingBox().width/2 - this.ball.x)*this.ball.speedY/(this.ball.speedX);
			...
			//초록박스 오른쪽 튕김 
			if(ballLeft > box_g_Right && ballLeft + this.ball.speedX < box_g_Right){
				//cc.log("Right");
				var xOnBox = this.ball.y + (box_g_Right + this.ball.getBoundingBox().width/2 - this.ball.x)*this.ball.speedY/(this.ball.speedX);
				var box_Top = this.box_g.y + this.box_g.getBoundingBox().height/2;
				var box_Bottom = this.box_g.y - this.box_g.getBoundingBox().height/2;
				if(xOnBox < box_Top && xOnBox > box_Bottom){
					cc.audioEngine.playEffect("res/bang.mp3", false);
